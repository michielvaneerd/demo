...
class HouseScreen extends StatefulWidget {
  ...
  const HouseScreen({required this.houseId, super.key});
...
class _HouseScreenState extends State<HouseScreen> {
      ...
      slivers: [
        SliverAppBar(
          ...
          flexibleSpace: FlexibleSpaceBar(
            title: showTitle ? Text(house.addess) : null,
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) =>
          HouseScreenCubit(Injections.houseUseCases)..getHouse(widget.houseId),
      child: BlocBuilder<HouseScreenCubit, HouseScreenState>(
        builder: (context, state) {
          return Scaffold(
            //appBar: AppBar(),
            body: _getWidget(state, context),
          );
        },
      ),
    );
